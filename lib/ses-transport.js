...
SESTransport.prototype.handleMessage = function (mail, raw, callback) {
    ...
    if (this.options.sesParams && typeof(this.options.sesParams) === 'object') {
        for (var option in this.options.sesParams) {
            console.log('Setting params[' + option + '] = ' + this.options.sesParams[option]);
            params[option] = this.options.sesParams[option];
        }
    }
